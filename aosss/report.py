    ...
    with open(fn_output, "w") as html:
        ...
        if  flag_par:
            ...
            try:
                ...
                flag_par_ok = False
                hpd.get_python_logger().exception("Failed to load file '%s'" % fn_par)
                html.write("(" + str(E) + ")")
        ...
        for item in items:
            ...
            try:
                ...
                hpd.get_python_logger().exception("Failed to dump header from file '%s'" % item.filename)
            ...
            try:
                ...
                if isinstance(item.fileobj, ao.FileFullCube) and (not "cube_seeing" in item.filename):
                    ...
                    hpd.set_figure_size(fig, FIGURE_WIDTH, 480. / 640 * FIGURE_WIDTH)
                    fig.tight_layout()
                elif isinstance(item.fileobj, hpd.FileSpectrum):
                    if item.keyword == "spintg":
                        sp_ref = item.fileobj.spectrum
                    fig = hpd.draw_spectra([item.fileobj.spectrum])
                    hpd.set_figure_size(fig, FIGURE_WIDTH, 270. / 640 * FIGURE_WIDTH)
                    fig.tight_layout()
                elif isinstance(item.fileobj, hpd.FileFits):
                ...
                hpd.get_python_logger().exception("Failed to load file '%s'" % item.filename)
        ...
        if  flag_log:
            ...
            try:
                ...
                hpd.get_python_logger().exception("Failed to dump log file '%s'" % fn_log)
                html.write("("+str(E)+")")
            html.write("</pre>\n")
        else:
...
def _draw_mask(filefits):
    ...
    width = HEIGHT/nr*nc
    hpd.set_figure_size(fig, width, HEIGHT)
    plt.tight_layout()
    return fig
