...
class MoveTask(object):
    ...
    def run(self):
        ...
        if not os.path.exists(dest_dir):
            hpd.get_python_logger().info("Creating directory '%s'..." % dest_dir)
            os.makedirs(dest_dir)
        hpd.get_python_logger().info("Moving '{}' to '{}'".format(os.path.relpath(self.source, cwd),
                                                               os.path.relpath(self.dest, cwd)))
        os.rename(self.source, self.dest)


if __name__ == "__main__":
    lggr = hpd.get_python_logger()
