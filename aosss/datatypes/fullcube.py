...
from astroapi import AttrsPart, get_python_logger, Spectrum, froze_it
import numpy as np
...
class FullCube(AttrsPart):
    ...
    def from_hdu(self, hdu):
        ...
        if not "CDELT3" in hdu.header:
            aa.get_python_logger().warning("HDU lacks header 'CDELT3', assumed 1.0")
            hdu.header["CDELT3"] = 1.
        if not "CRVAL3" in hdu.header:
            aa.get_python_logger().warning("HDU lacks header 'CRVAL3', assumed 0.0")
            hdu.header["CRVAL3"] = 0.
        if not "CDELT1" in hdu.header:
            aa.get_python_logger().warning("HDU lacks header 'CDELT1', assumed 1.0")
            hdu.header["CDELT1"] = 1.0
        if not "CDELT2" in hdu.header:
            aa.get_python_logger().warning("HDU lacks header 'CDELT2', assumed value in 'CDELT1'")
