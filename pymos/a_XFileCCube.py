...
VVV = FileCCube.description

class NullEditor(object):
    f = None

################################################################################
class XFileCCube(QMainWindow):
    """
    Arguments:
      parent=None -- nevermind
      file_main (optional)-- FileCCube instance
    """
    ...
    def __init__(self, parent=None, file_main=None, file_abonds=None):
        ...
        self.tab_texts =  ["FileCCube editor (Alt+&1)",
                            "Sth else #0 (Alt+&2)",
                            "Sth else #1 (Alt+&3)"]
        self.flags_changed = [False, False, False]
        self.save_as_texts = ["Save %s as..." % VVV,
                               "Save Sth else #0 as...",
                               "Save Sth else #1 as..."]
        self.open_texts = ["Load %s" % VVV,
                            "Load Sth else #0",
                            "Load Sth else #1"]
        self.clss = [FileCCube, None, None]
        self.wilds = ["*.fits", "*.py", "*.py"]
...
##        tt.tabBar()
        ...
        # ~~~
        self.editors = [self.ce, NullEditor(), NullEditor()]
        self.labels_fn = [self.label_fn_ccube, self.label_fn_sth0, self.label_fn_sth1]
        ...
        # # Loads default files
        if os.path.isfile(FileCCube.default_filename):
            f = FileCCube()
            f.load()
            self.ce.load(f)
